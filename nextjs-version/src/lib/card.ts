...
export function generateEmotionCard(canvasId: string, sessionData: CardData) {
  ...
  ctx.textAlign = 'center';
  ctx.fillStyle = '#b8a99a';
  ctx.fillText('晴愈AI · 直面情绪 向阳而生', W / 2, cardY + cardH - 30);
}

export function downloadCard(canvasId: string, filename?: string) {
  const canvas = document.getElementById(canvasId) as HTMLCanvasElement;
  if (!canvas) return;
